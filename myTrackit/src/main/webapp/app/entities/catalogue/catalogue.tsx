...
export const Catalogue = () => {
  ...
  const catalogueList = useAppSelector(state => state.mytrackit.catalogue.entities);
  ...
  return (
    <div>
      <h2 id="catalogue-heading" data-cy="CatalogueHeading">
        <Translate contentKey="myTrackitApp.catalogue.home.title">Catalogues</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="myTrackitApp.catalogue.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/catalogue/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="myTrackitApp.catalogue.home.createLabel">Create new Catalogue</Translate>
          </Link>
        </div>
      </h2>
        ...
        {catalogueList && catalogueList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="myTrackitApp.catalogue.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {catalogueList.map((catalogue, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{catalogue.materialCode}</td>
                  <td>{catalogue.materialDesc}</td>
                  <td>{catalogue.materialGroup}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="myTrackitApp.catalogue.home.notFound">No Catalogues found</Translate>
            </div>
          )
        )}
      </div>
